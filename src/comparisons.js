...
export function createComparisonsWidget(containerId, options) {
    ...
    function drawLegend() {
        ...
        const totalLegendWidth = maxTextWidth + colorBoxSize + textMargin;
        ...
        options.distributions.forEach((distribution, index) => {
            ...
            const y = legendStartY + index * (legendItemHeight + legendItemSpacing);

            // Draw color box
            ctx.fillStyle = colorScheme.stroke;
            ctx.fillRect(legendX, y + 2, colorBoxSize, colorBoxSize);
            ...
            ctx.fillText(text, legendX + colorBoxSize + textMargin, y + colorBoxSize / 2 + 2);
        });
    }
